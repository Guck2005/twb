...
function initQuiz() {
  currentIndex = 0;
  answers = quiz.items.map((item) => (isMulti(item) ? new Set() : null));
  validated = new Set();
...
function render() {
  ...
  quizCounterEl.textContent = `Question ${item.num} / ${total}`;
  ...
  if (multi && item.correct.length > 1 && !validated.has(currentIndex)) {
    ...
    hint.textContent = `${item.correct.length} r\u00e9ponses attendues`;
  ...
  item.options.forEach((opt, idx) => {
    ...
    label.querySelector(".answer-option__text").textContent = opt;
